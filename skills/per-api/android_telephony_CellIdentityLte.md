...
|---|---|---|---|---|---|---|
| `getBandwidth` | `int getBandwidth()` | 7 | near | moderate | `getRadioTech` | `getRadioTech(slotId: number, callback: AsyncCallback<NetworkRadioTech>): void` |
| `getCi` | `int getCi()` | 7 | near | moderate | `getIMEI` | `getIMEI(slotId: number, callback: AsyncCallback<string>): void` |
| `getPci` | `int getPci()` | 6 | near | moderate | `getIMEI` | `getIMEI(slotId: number, callback: AsyncCallback<string>): void` |
| `getTac` | `int getTac()` | 6 | partial | moderate | `getSimState` | `getSimState(slotId: number, callback: AsyncCallback<SimState>): void` |
| `getEarfcn` | `int getEarfcn()` | 5 | partial | moderate | `getMaxSimCount` | `getMaxSimCount(): number` |
